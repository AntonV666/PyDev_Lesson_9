...
class ShoppingCart:
    ...
    def get_details(self, customer_name, admin_name):
        """
        Возвращает детализированную информацию о содержимом корзины и общей стоимости.
        """
        details = "Корзина покупок:\n"
        for item in self.items:
            details += f"Покупатель: {customer_name}, приобрел {item['Продукт'].get_details()}, Количество: {item['количество']}, Зарегистрировал покупки пользователь: {admin_name}\n"
